...
# Absolute location of this pipeline code (captured before any os.chdir calls)
_PIPELINE_SCRIPT_PATH = Path(__file__).resolve() if '__file__' in globals() else None
_PIPELINE_SCRIPT_DIR  = _PIPELINE_SCRIPT_PATH.parent if _PIPELINE_SCRIPT_PATH else None
...
class PipelineConfig:
    def __init__(self):
        ...
        # Provenance
        # Determine repo_root robustly for both repo-root and unified_pipeline_clean placements
        if _PIPELINE_SCRIPT_DIR:
            # If we're under unified_pipeline_clean/nif_downstream_code or unfied_pipeline_clean/nif_downstream_code
            if _PIPELINE_SCRIPT_DIR.parent.name in ('unified_pipeline_clean', 'unfied_pipeline_clean'):
                self.repo_root = _PIPELINE_SCRIPT_DIR.parent.parent
            else:
                # If we're under repo_root/nif_downstream_code
                self.repo_root = _PIPELINE_SCRIPT_DIR.parent
        else:
...
def load_and_filter_assemblies(assembly_quality_path: Path, output_dir: Path):
    ...
    # NCBI assembly quality exports may use either "accession" (common in our
    # upstream scripts) or "assembly_accession" (used by some older pipeline
    # tables). Normalize to a single "assembly_accession" column here.
    acc_col = None
    for cand in ["assembly_accession", "accession", "Assembly Accession", "assembly accession"]:
        if cand in df.columns:
            acc_col = cand
            break
    if acc_col is None:
        raise RuntimeError(f"No accession column found in assembly quality table; columns={list(df.columns)}")

    df["assembly_accession"] = df[acc_col].astype(str).str.strip()
    level_col = None
...
def _find_script(name: str) -> Optional[Path]:
    """Locate helper step scripts robustly regardless of current working directory.

    The pipeline may `chdir()` into the run output directory. If `__file__` is relative,
    resolving it after a chdir can point to the wrong place. We therefore anchor searches
    to `_PIPELINE_SCRIPT_DIR` captured at import time, with additional fallbacks for common
    repo layouts and an optional environment override.
    """
    tried: List[Path] = []
    candidates: List[Path] = []

    # Optional override: explicitly pin where step scripts live
    override = os.environ.get("PANGENOME_SCRIPTS_DIR") or os.environ.get("PANGENOME2_SCRIPTS_DIR")
    if override:
        candidates.append(Path(override).expanduser().resolve() / name)

    # Scripts alongside this consolidated pipeline and/or its parent folder
    if _PIPELINE_SCRIPT_DIR:
        candidates.append(_PIPELINE_SCRIPT_DIR / name)
        candidates.append(_PIPELINE_SCRIPT_DIR.parent / name)

    # Common repo layouts (git checkout or extracted zip)
    rr = getattr(config, "repo_root", None)
    if rr:
        rr = Path(rr).resolve()
        candidates.extend([
            rr / name,
            rr / "nif_downstream_code" / name,
            rr / "unified_pipeline_clean" / "nif_downstream_code" / name,
            rr / "unfied_pipeline_clean" / "nif_downstream_code" / name,
        ])
    ...
    print(f"ERROR: {name} not found. Paths tried:")
    for pth in tried:
        print(f"  - {pth}")
    return None
...
def step6_merge_annotations():
    # Merge annotations if prerequisite inputs exist
    if not os.path.exists("gf_reps_vs_sprot.tsv"):
        return True

    script_path = _find_script("06_merge_annotations_selected.py")
    if not script_path:
        print("WARN: 06_merge_annotations_selected.py not found; skipping Step 6.")
        return True

    try:
        subprocess.run([sys.executable, str(script_path)], check=True)
    except subprocess.CalledProcessError as exc:
        print(f"ERROR: Step 6 failed: {exc}")
        return False
...
def step7_expand_gene_families():
    script_path = _find_script("07_expand_gene_families.py")
    if not script_path:
        print("WARN: 07_expand_gene_families.py not found; skipping Step 7.")
        return True

    try:
        subprocess.run([sys.executable, str(script_path)], check=True)
    except subprocess.CalledProcessError as exc:
        print(f"ERROR: Step 7 failed: {exc}")
        return False
...
def step8_build_narrative_table():
    script_path = _find_script("08_build_narrative_table.py")
    if not script_path:
        print("WARN: 08_build_narrative_table.py not found; skipping Step 8.")
        return True
    ...
    try:
        ...
        print(f"ERROR: Step 8 failed: {exc}")
        return False
